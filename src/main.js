...
import BSTCore from "./core";

import { permanent, constant } from "./object-attributes";
...
export default class BinarySearchTree {
    ...
    */
    constructor(options) {
        ...
        const core = new BSTCore(options);
        g.set(this, core);

        constant(this, "dataType", core.controllers.dataType);
    }
    ...
    getMin(node = this.root) {
        if (!is.node("getMin", node)) return null;

        return g.get(this).getMin(node);
    }

    getMax(node = this.root) {
        if (!is.node("getMax", node)) return null;

        return g.get(this).getMax(node);
    }

    getHeight(node = this.root) {
        if (!is.node("getHeight", node)) return null;

        return g.get(this).getHeight(node);
    }

    isBalanced(node = this.root) {
        if (!is.node("isBalanced", node)) return null;
    ...
    */
    ...
    destroy() {
        ...
        g.delete(this);
        g.set(this, new BSTCore(options));
    ...
    getData(walkingType = "inorder", node = this.root) {
        ...
        walkingType = is.traversalType(walkingType);

        is.node("getData", node);
    ...
    */
    ...
    merge(...trees) {
        ...
        trees.forEach((tree, index) => {
            if (!is.object(tree) || !is.constructor(tree, BinarySearchTree)) {
            ...
            if (core.validTree(tree)) {
                ...
                throw `Can't merge two binary search trees with different data type: ${this
                    .controllers.dataType} !== ${tree.controllers
